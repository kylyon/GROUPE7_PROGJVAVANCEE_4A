...
public class TEXTV1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (target.position != transformTest.position)
        {
            Debug.Log("Next Step");
            ComputeMCTS();

            foreach (var possibleAction in allPossibleActionsMove)
            {
                allPossibleActionsResult[possibleAction.Key] = 0;
            }
        }
    ...
    void ComputeMCTS()
    {
        ...
        transformTest.position += new Vector3(allPossibleActionsMove[bestAction].Item1, 0, allPossibleActionsMove[bestAction].Item2);
    }
    ...
    int SimulateResult(Action possibleAction)
    {
        ...
        positionTemp += new Vector3(allPossibleActionsMove[possibleAction].Item1, 0, allPossibleActionsMove[possibleAction].Item2);
        ...
        while (positionTemp != target.position) //Attention votre jeu doit être fini !
        {
            ...
            positionTemp += new Vector3(allPossibleActionsMove[actions[selectedAction]].Item1,0, allPossibleActionsMove[actions[selectedAction]].Item2);
